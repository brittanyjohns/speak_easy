class ResponseBoardsController < ApplicationController
  ...
  def index
    @response_boards = ResponseBoard.includes(response_images: [image: [cropped_image_attachment: :blob, saved_image_attachment: :blob]]).order(created_at: :desc).page params[:page]
  end
